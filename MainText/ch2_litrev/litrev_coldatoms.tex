...
Bose--Einstein condensation was predicted by N. Bose and A. Einstein \cite{BEC:Ketterle_revmod_2002}, and began as a work on the statistical behaviour of photons. This eventually led to their derivation of the ``Bose--Einstein distribution'' for an ideal bosonic gas. In the framework of the grand canonical ensemble and following the description given by Pitaevskii and Stringari \cite[chap. 2]{BK:Pitaevskii_Stringari_2003}, the Bose--Einstein distribution is given by
\begin{equation}
...
\end{equation}
Substituting Eq.~\ref{eqn:gpe_fluc} into \ref{eqn:ham2ndq}, taking only terms in $\Psi$, and following the above procedure gives the time dependent mean-field Gross--Pitaevskii equation (GPE) as
...
Assuming a zero-temperature condensate, the number of uncondensed atoms will be zero, and thus we can safely ignore all terms in $\delta\hat{\Psi}$. The remaining condensed term is of the form of a classical field, wherein the problem moves from a quantum one to semiclassical. The time independent form can be found by evaluating the left-hand side derivative and dividing across by $e^{-\frac{i\mu t}{\hbar}}$, yielding
\begin{equation}
...
\end{align}
\end{subequations}
where I have linearised the resulting equations in terms of $u$ and $v$. Plugging these terms into the GPE yields the Bogoliubov-de Gennes equations,
